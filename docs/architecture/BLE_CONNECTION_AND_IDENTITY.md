...
4. 广播 Device ID 和屏幕型号只能筛选候选，不能完成最终验身——**2026-08-04 起新固件广播已是完整 6 字节，这一条仍然成立**（广播是明文，可被伪造重放；只有 0x01 是从设备真身读回来的）。
...
| 广播 Device ID | 厂商广播数据 | 扫描候选筛选 + 列表展示 | 老固件 4 字节（同批次可能重复）；**2026-08-04 起新固件为完整 6 字节**，但**仍不得用于验身**——广播明文可伪造重放 |
