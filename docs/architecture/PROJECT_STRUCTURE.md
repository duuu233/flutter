...
不存在独立的 `features/album` 或 `features/demo` 模块。「我的相册」能力位于 `gallery`，演示与
...
| `mine` | 个人中心与「我的相册」、设备、指南、设置入口（2026-08-04 起不再有独立的投屏管理入口） |
| `account` | 邮箱/微信登录、注册、找回/修改密码、资料、邮箱变更 |
| `devices` | 扫描绑定、设备列表/详情、轮播、清空、解绑、OTA、BLE 调试与性能自检 |
| `cast` | 选图、预览编辑、后端转帧、BLE 图传、结果页和投屏记录 |
| `gallery` | 「我的相册」：按设备分类展示投屏成功的照片、批量再次投屏与删除（2026-08-04 由「设备照片」+「投屏管理」合并） |
