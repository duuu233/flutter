...
## 2026-07-31 设备名称上限与保存态

- 小程序与 Flutter 的设备名称上限统一为 20 个 Unicode 字符；两端均按码点计数，emoji 不重复计数。
- Flutter “我的设备”和“设备详情”两个保存入口增加共用的阻断式“保存中” loading，`finally` 负责收口。
- 改名仍是纯展示字段更新，不参与完整设备 ID、BLE 句柄或活动会话匹配。
- 详细记录：`2026-07-31-设备改名保存Loading.md`。
- 按用户指示在原需求第 16 项完成后停止。Flutter 明确未完成：第 1 项绑定后命名与全入口省略号复核，
  第 6、8、9、11、12、13、15、17 项；第 3、4、5、7、10、14 项是否要求 Flutter 完全同步尚待确认。
  逐项停止点见 `2026-07-31-设备改名保存Loading.md`。
