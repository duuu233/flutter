...
| 文档 | 主职责 |
| --- | --- |
| [`runbooks/BUILD_RELEASE.md`](runbooks/BUILD_RELEASE.md) | Android/iOS 构建、签名、占位符与发布检查；**环境与依赖基线（Flutter 3.44.6，不是必须就不升级、不动依赖，本机环境带出来的改动不提交）** 与 9-18 升级的回退（依赖已回退，iOS 部分待 ltt，〇.6、〇.7） |
...
| [`history/2026-09/2026-09-02-安卓六宫格副标题再小一号.md`](history/2026-09/2026-09-02-安卓六宫格副标题再小一号.md) | Historical Change Record（**仅 App 端、且只影响安卓**：首页 3×2 六宫格副标题**再小一号**，`_entrySubtitleAndroidFinalDelta` **1 → 2**，安卓最终字号 7~9 → **6~8**，与 09-02 第六轮逐像素相同；iOS 一个像素不动。⚠️ **这个档位已经在 1 与 2 之间来回一趟**（④+1 → ⑤+2 → ⑥−1 → ⑦−1 → ⑧+1），360dp 屏上就是**中文 8 与 7 的取舍、两档之间没有别的整数** —— 若真机上「8 偏大、7 偏小」不必再来回，这个常量吃 **0.5**（第一/二步本就按 0.5 步进量），`1.5` 在 360dp 上给出 7.5。本轮顺手把那个常量上逐轮追加的散注释**收成一张轨迹表**并写清取值范围 `[0, 3]` 非负 —— 五轮下来注释已比代码长。安全论证不变：`result` 是量出来的「放得下的字号」，最终值 `result - finalDelta` 只要差值 ≥ 0 就不越过它，上下调档数都安全，**但差值不能为负**（中文折行、英文出「...」）。实测预期（中文）：360dp 8→**7**、384/390/412dp 9→**8**，全部仍单行；下限 `9−1−2=6`。两端对照：iOS 9~11、安卓 6~8，小 3 号。🔶 小程序不动。⚠️ **未编译未真机**） |
| [`history/2026-09/2026-09-03-简中语种不支持PayPal支付.md`](history/2026-09/2026-09-03-简中语种不支持PayPal支付.md) | Historical Change Record（**仅 App 端、且只影响安卓**：确认购买页点「立即购买」时，**简中语种**只弹一句「暂不支持人民币付款，请切换其他语种进行支付」并当场返回，建单/创建支付/跳 PayPal 一步都不走；繁中 / English / 日本語 完全不变。判定收在 `StarPurchase.payPalBlockedFor`（全 App 只有这一处判这件事，将来 PayPal 开通人民币改这一个方法），文案是 `AppL10n.starBuyCnyUnsupported`（简中按需求原文逐字，其余三语种的译文实际看不到、只为补齐 `_pick`）。⚠️ **拦截点必须是 `_buy()` 的第一步**：放到 `AppLoadingDialog.show` 之后会先闪一下「正在下单…」像是失败，放到 `StarPurchase.start()` 里则**已经建了单** —— 后台留一张永远付不掉的待支付单，正是 iOS 那条「付不了就别建单」（`StarPayType.supportedOnThisApp`）踩过的坑。⚠️ **只拦简中**，别顺手把 `AppLanguage.zhHant` 算进来（需求原文就是「简中用户不支持 paypal 支付」）。⚠️ 购买按钮**有意不置灰**：这条限制的出路是「换个语种」，得让用户点得到、听得见那句提示；提示走 `AppToast.warn`（与本页其它「这次买不成」的话同一个出口，弹窗只留给「已取消支付」那种终局结论）。用例钉的是拦截范围而非文案。🔶 小程序不动（那边是微信支付，本就收人民币）。⚠️ **未编译未真机**：本机无 Flutter SDK） |
| [`history/2026-09/2026-09-03-首页设备卡圆环与文字上下居中.md`](history/2026-09/2026-09-03-首页设备卡圆环与文字上下居中.md) | Historical Change Record（**仅 App 端**：首页设备轮播卡里「左侧圆环与右侧文字没上下居中」——`Row` 的居中没问题，偏的是右列那个写死的 `SizedBox(height: 110)`：盒子居中、盒里的 `Column` 顶对齐，而内容只有 84.4 高（已连接）/95.4（未连接），空档全落在下面，内容中心比卡中心**高 12.8 / 7.3**，且该偏移与屏宽无关。⚠️ **与 iOS 无关**：卡内字号全写死 `height: 1.2`，行盒两端逐像素一致，安卓同样偏，只是这轮在 iOS 真机走查看出来。110 = 小程序 `.device-info` 的 `min-height: 220rpx`，那条是为「切换连接态时文字不上下跳」而设（块高恒定 + 内容顶对齐），**但它是按 webview 默认行高（≈1.4）量的**，Flutter 写死 1.2 后同样的内容只有 169rpx，照抄就空一截。改法两条必须同时做：① 右列改 `Column(mainAxisSize: min)`，由 `Row` 默认的 `center` 对齐**内容**（84.4 ≈ 圆环 83，两边几乎等高）；② 状态行下方那条间距两态统一走新常量 `_kCardMetaGap = 11` 并**提到 `if` 外面**——电量行高 26 与「连接蓝牙」按钮高 26（7+12+7）恰好相等，间距一致 → **两态逐像素等高**，连接成功那一刻设备名一个像素都不动，比原来的「块高恒定」更硬。⚠️ 只做 ① 不做 ② 等于拿「文字上下跳 5.5」换居中。顺带修掉未连接态那个 `22`：小程序 `.device-status` 的 `margin-bottom: 22rpx` 与 `.device-connect-btn` 的 `margin-top: 22rpx` 是相邻兄弟、**外边距合并**成 22rpx(=11)，App 原来把两条 margin 加了起来。⚠️ 下一轮若嫌未连接态按钮挤，改 `_kCardMetaGap` **一处**、两态一起动，别只加未连接那边。卡片比例 654:298、24 圆角、`_CardGlass` 投影、`_kCardIconGap`、圆环 83 一个数都没动；右列不再固定高度反而更抗系统字体放大（原来超过 110 就在盒子里 overflow，现在上限是卡高 ≈143）。`_ConnectedDeviceCard` 是文件私有 widget、无可注入入口，**未新增用例**。🔶 小程序不动。⚠️ **未编译未真机**：本机无 Flutter SDK） |
| [`history/2026-09/2026-09-03-首页设备卡左右弹性布局.md`](history/2026-09/2026-09-03-首页设备卡左右弹性布局.md) | Historical Change Record（**仅 App 端**：首页设备轮播卡**照抄小程序的左右弹性布局**——`.device-carousel` 是 `justify-content: space-around` 的 flex 行，两个孩子（圆环定宽 166rpx、`.device-info` 没写 width ⇒ **内容宽**，设备名被 `max-width: 320rpx` 封顶）各取内容宽，剩余空档摊成**「外 1 : 中 2 : 外 1」**，整组在卡内水平居中。App 改前是 2026-08-31 留下的「圆环贴左内缩 18 + 写死 `_kCardIconGap` 16 + 右列 `Expanded` 吃满」：`Expanded` 意味着**没有剩余空间可摊**，`MainAxisAlignment` 写什么都没用，375dp 上左留白 18 / 中间 16 / **右边空 118**，就是需求说的「左右的内容都偏左」。改成 `spaceAround` + 右列 `ConstrainedBox(maxWidth: 卡宽 − 圆环 − 2×_kCardIconGap)` 取内容宽（375dp 名字 80 时 38/76/38，小程序同口径 41/82/41，差的 12 是 App 卡为 `_kCardPeekGap` 让出的那条缝）。⚠️ **四处配套少一处就白改**：右列必须放弃 `Expanded`；列里两条 `Row` 补 `mainAxisSize.min`（默认 `max` 会撑满上限、把空档吃回去）；「连接蓝牙」按钮**不再套 `Align`**（`Align` 在有界约束下摊满，同样把列钉死在上限宽）；卡内去掉 `horizontal: 18` 内边距（留白改由 spaceAround 摊，否则成「18 + free/4」，比小程序更靠里且左右不等）。⚠️ **窄屏不会重演 2026-08-31 那次「贴到一起」**：那次的病根是右列写死 `maxWidth: 190`（360dp 上 83+190 顶满 276、中间只剩 1.5px），这轮的闸在**信息块宽度上限**而不是间距——顶到上限时 free 恒为 32、摊出的中间间距恰好 = `_kCardIconGap` 16，再窄就先省略设备名；屏宽 ≥ 335dp 时先顶到的是设备名自己的 160（新常量 `_kCardTitleMaxW`，即小程序 320rpx），360dp 中间仍有 28.5。顺手给状态行文案补 `Flexible` + 省略号（右列比改前窄 16，长译文不再有溢出风险）。⚠️ **取舍**：设备名短时中间会拉开约 76，这是 space-around 的定义、也是小程序线上的样子；若产品要「贴近点」应改 `center` + 固定间距（**有意偏离**小程序），别去改 `_kCardIconGap`（它只是窄屏下限，改大只会更早省略设备名）。同日纵向那轮（圆环与文字上下居中）的结论未被触动，卡片比例 654:298、24 圆角、`_CardGlass` 投影、圆环 83、`_kCardMetaGap` 11 一个数都没动。**未新增用例**（文件私有 widget，无可注入入口）。🔶 小程序不动。⚠️ **未编译未真机**：本机无 Flutter SDK） |
| [`history/2026-07/RUNBOOK_CHANGELOG.md`](history/2026-07/RUNBOOK_CHANGELOG.md) | Historical Change Record |
| [`history/2026-09/2026-09-16-启动页slogan改呈现美好.md`](history/2026-09/2026-09-16-启动页slogan改呈现美好.md) | Historical Change Record（**仅 App 端**：闪屏 LOGO 下的 slogan 由硬编码的「把美好，留在一张纸上」改为走 `AppL10n` 的 `splashSlogan`——简中「呈现美好」/ 英文 `Present Beauty` / 日文「美しさを映す」；**繁中不单独给**，`_pick` 缺省走 `toTraditionalChinese`，字表第 309 行 `0x73B0: 0x73FE`（现→現）、其余三字繁简同形，结果正是「呈現美好」。闪屏位于 `MaterialApp.builder` 的 `AppLocalizationsScope` 之内，能正常拿到当前语言，接入后去掉了外层 `const`。⚠️ 原生冷启动图（Android `launch_background.xml` / iOS `LaunchScreen.storyboard`）**只有背景与 LOGO、不含文案**，所以 Flutter 首帧这一处就是唯一显示点，无需同步原生资源。⚠️ 注册页副标题 `accRegisterSubtitle` 仍含旧 slogan，本轮按需求只改启动页。🔶 小程序侧没有这句文案。⚠️ **未编译未真机**：本机无 Flutter SDK） |
| [`history/2026-09/2026-09-16-搜索设备列表4.5张与首页宫格精简.md`](history/2026-09/2026-09-16-搜索设备列表4.5张与首页宫格精简.md) | Historical Change Record（**双端同步**，两件事。① **搜索设备结果页**：产品要「6 台设备时列表显示 4.5 张、最后一张露一半表示能滑」。改前列表是 `Expanded` 吃剩下的高度、雷达雷打不动占 280，iPhone 13 上正好 3 张多——就是需求说的「目前只显示 3 个」。改成雷达与列表在同一个 `LayoutBuilder` 里一次算完：**列表先拿够 `_kListViewport = 82×4 + 12×4 + 41 = 417`（4 张整 + 半张），雷达拿剩下的并夹在 `_kRadarMin` 150 ~ `_kRadarMax` 280**，屏幕矮到连雷达下限都保不住时才反过来压列表。顺带把统计行固定成 44（原来扫描停止时从 42 跳到 28，整页会往上跳）。实得：iPhone 13 雷达 150/列表 413≈4.45 张，大屏安卓 242/417=4.5 张，小屏 SE 降级到 2.4 张（改前约 0.9）。⚠️ 雷达在 iPhone 13 上会从 280 缩到 150，是把高度让给列表的代价；嫌切换时跳就上调 `_kRadarMin`，代价是列表少半张。② **首页六宫格**：按产品「去掉副标题和右侧箭头、把图标拉大、主标题字号不变」，`_HomeEntryCard` 删 `subtitle`/`subtitleFontSize`/`arrowAsset` 三个入参，图标 33 → **48**、与标题的间距 6 → 10、内边距左右对称回 10（右边那 6 原是给箭头透明阴影留的），**标题字号基准一个数没动**；卡片 `AspectRatio(206/220)` 与卡高不变（副标题腾出的高度正好给图标）。随之删掉 `_entrySubtitleFontSize` / `_subtitlesFitInLines` 与三个安卓微调常量共 **≈190 行**——那是 09-01~09-02 四五轮「副标题再大/再小一号」叠出来的机器，副标题没了就没有消费方；`_HomeTextStyles.entrySubtitle` 一并删（私有类里的死字段会被 analyzer 报）。六张箭头素材 `home-icon11~16.png` 留仓备回滚、进 `unused_assets_test` 的 parked 白名单。🔶 小程序侧同日同口径已改（`bind.wxss` 竖向 flex + `max-height: 844rpx`；`home.wxss` `.entry-icon` 66 → 96rpx）。⚠️ **推上去后 release 打包失败过一次**：改 `home_text_styles.dart` 时按「注释段 → 下一个定义」整段替换，把与 `entrySubtitle` 挨在一起、共用同一段注释的 **`entryTitle` 一起带走了**，三处引用报 `Member not found`。`3d39f91` 原样补回（字号一个数没动）。根因是当时的 grep 只做了一个方向——查「要删的符号还有没有人引用」（全 0，看着很干净），没查「还有人引用的符号是不是都还在」。此后补了三条静态自检：`_HomeTextStyles.*` 引用与定义两头对齐、`git diff` 删除清单逐个查活引用、构造函数命名参数与全部调用点比对。⚠️ **本机与内网 nas 均无 Flutter SDK**，`dart analyze` / `flutter test` / 编译 / 真机仍未跑；小程序同款改动跑过 Node 用例可作交叉印证） |
| [`history/2026-09/2026-09-16-首页宫格图标缩小与居中.md`](history/2026-09/2026-09-16-首页宫格图标缩小与居中.md) | Historical Change Record（**仅 App 端**，同日宫格改版的第二轮：产品复看后要「把图标缩小、图标和文字居中」。`_HomeEntryCard` 三处——`_iconSize` **48 → 45**（对齐小程序 `.entry-icon` 96 → **90rpx**，折算沿用卡里其余尺寸的 **rpx 折半** 口径：同卡 `border-radius: 28rpx` → 14、`margin-bottom: 20rpx` → 10）、`CrossAxisAlignment.start` → **center**、标题补 `textAlign: TextAlign.center`。390dp 屏上纵向只是上下留白 19.7 → 21.2（缩掉的 3 由整组垂直居中自动摊回，**卡高与 206/220 比例没动**），横向才是观感大头：图标左边距 0 → 21.5、标题 0 → 14，**图标与标题各自按自身宽度居中、不左对齐成一条竖线**（与小程序 `align-items: center` 同）。⚠️ **居中不牵动标题字号**：`CrossAxisAlignment` 从 start 换到 center，Column 给孩子的都是松约束 `0 ~ 内容宽`，`_entryTitleFontSize` 的输入 `available = 卡宽 − titleHorizontalReserve` 一个数不变，日文「マイアップロード」照旧压在 ≈11.5，省略号阈值没移；`textAlign` 平时不产生位移（标题框按内容宽收缩），只在标题被截断、文本框顶满卡宽那一刻保证仍居中。🔶 **小程序侧由产品自己改**，⚠️ 写记录时那次改动**还没推到远端**（`photo-album` 的 `home.wxss` 仍停在 `804b82e` 的 96rpx、无 `align-items`），等推上来要核对实际值；落在别的数就只改 `_iconSize` 这一个常量。⚠️ **未编译未真机**：本机与内网 nas 均无 Flutter SDK；已做反向引用、删除清单、构造函数参数、括号配对四项静态自检） |
| [`history/2026-09/2026-09-16-日文六宫格标题被截成省略号.md`](history/2026-09/2026-09-16-日文六宫格标题被截成省略号.md) | Historical Change Record（**仅 App 端**：日文版首页六宫格**只有第三张卡**显示「マイアップロ…」。根因不是宽度算错，是**算出来的字号让最长那条恰好占满、余量为 0**——六张卡共用一个字号、由「六条里最长的那条」反推，日文 `マイアップロード` 是 8 个全角字（其余五条 ≈3.1~6），于是字号 = `可用宽 ÷ 8`、文字宽**等于**可用宽（360/375/390/412/480dp 上余量一律 **0**），其余五条都有 ≥ 2 个字富余，所以偏偏只有它被截。而「按比例缩」本身不保证严丝合缝：估算用 `TextPainter..layout()` 量固有宽，渲染走的是「`maxLines` + 省略号 + 约束宽」那条路径，边界上差半个逻辑像素就截断；系统字体缩放**非线性**时（Android 14+）比例关系更是直接不成立（估算里 `textScaler` 乘在基准字号上、渲染时乘在结果字号上）。**改法**：第一步的比例估算保留不动，后面接一段**按 widget 真实条件的复测**——新增 `_entryTitleOverflows` 把 `TextPainter` 摆成和卡里那个 `Text` 逐项相同的样子（`maxLines: 1` + 同一个省略号 `\u2026` + `layout(maxWidth: 可用宽)`），直接问 `didExceedMaxLines`，截就退 `_entryTitleStepDown`（0.25 号 = 让开 2 逻辑像素）。日文退一步即通过：390dp 上 11.0 → **10.75**、余量 2.00，肉眼无差。⚠️ **下限取相对值**（估算值 × 0.9）而不是写死字号——窄屏估算值本来就小（320dp 日文约 8），写死绝对下限会让那些机器一步都退不了、截断照旧。中文四字、英文（`Camera Cast` 早已缩成 `Camera`）本来就有富余，**复测一次就通过、字号一个数不变**。日文文案没有改短（产品要的就是「按它展示完整」）。🔶 **小程序无此问题**：那边 `HOME_ENTRIES` 的 `name` 写死中文，语种只决定后端返回内容的语言。⚠️ **未编译未真机**：本机与内网 nas 均无 Flutter SDK；已做反向引用、删除清单（本轮纯新增）、新符号两头对齐、括号配对四项静态自检） |
| [`history/2026-09/2026-09-16-日文六宫格标题第二修-省略号结构性消除.md`](history/2026-09/2026-09-16-日文六宫格标题第二修-省略号结构性消除.md) | Historical Change Record（**仅 App 端**，上一条的**二修**——一修推上去后产品反馈「还是没展示完整」。**一修没修好的原因：量的样式不对。** `Text` 渲染用的是 `DefaultTextStyle.of(context).style.merge(style)`（见 `Text.build`），而 `TextPainter` 只喂了 `_HomeTextStyles.entryTitle` 自己那份，漏掉主题带来的 **`letterSpacing: 0.25`**（`bodyMedium` 继承 Material `englishLike2021`；⚠️ 日文 locale 走 `dense` 几何，但 dense 的 `letterSpacing` 是 null，`TextStyle.merge` 只覆盖非空字段，**0.25 原样保留**）与 `fontFamily: Roboto`。8 个全角字就是 **+2.0 逻辑像素**没量进去：390dp 上量 88 说放得下、实际画 90；一修退 0.25 号后量 86 / 实际 88，**正好顶死、仍然截断**。⚠️ 更根本的是 **`letterSpacing` 是加法项、不随字号缩放**，所以「宽度与字号成正比」从根上不成立，一修那套「比例外推 + 退档」怎么退都差一口气。**三处改动**：① 估算与复测统一走新增的 `_entryTitleStyle`（= `DefaultTextStyle.of(context).style.merge(entryTitle)`），⚠️ 前提是宫格到 `Text` 之间没有别的 `DefaultTextStyle`，将来套了就得把样式往下传；② `_entryTitleOverflows` 改成**在候选字号上实测**再比预算，并新增 `_entryTitleSafetyMargin = 1`，**不允许「正好等于」**；③ 产品原则「就是不要出现 …」落成**结构保证**——卡内左右内边距 `_padLeft`/`_padRight` **10 → 6**（两件都横向居中后，内边距只剩「给标题封顶宽度」一个作用，🔶 与小程序 20rpx 有意偏离），标题由 `Text(overflow: ellipsis)` 换成**`FittedBox(scaleDown)` + 不设 `overflow`**：没配省略号就不可能画出 `…`，算准时 scaleDown 什么都不做、六张卡仍一样大，算多了也只是那一条等比缩几个百分点。⚠️ 别改回 `ellipsis`——`FittedBox` 里 `Text` 拿的是无界宽约束，永远不会触发。**结果**（日文 8 全角字，均留 2 余量）：390dp 字号 **11.430**、实宽 94/可用 96；360dp 10.201、412dp 12.332、480dp 吃满基准 **15**——比产品现在装的那版（11.0、实宽 90、截断）还大。中文/英文字号一个数不变，日文文案未改短。⚠️ **未编译未真机**：本机与内网 nas 均无 Flutter SDK；五项静态自检（反向引用、删除清单、新符号两头对齐、构造函数参数×6 调用点、括号配对） |
...
| Wi-Fi 版相框 / BluFi 配网可行性 | `integration/WIFI_FRAME_BLUFI_ANALYSIS.md` |
...
- [2026-09-24 iOS 隐藏微信快捷登录](history/2026-09/2026-09-24-iOS隐藏微信快捷登录.md)：上次 iOS 审核因「有第三方快捷登录却没有苹果登录」不通过，方案是 iOS 包不露微信快捷登录、只留邮箱登录，安卓不变。开关一处定义 `weChatLoginHiddenOnThisApp`（`defaultTargetPlatform`，便于 widget 测试切端），生效点只有登录页底部的微信按钮（全应用唯一入口）；`fluwx` 依赖与 SDK 注册不动。用例 `test/auth_page_test.dart` 增 iOS/安卓两条。⚠️ 本机无 Flutter 工具链，analyze/test 未跑。
- [2026-09-21 三项问题双端核对：一键清空不再干等 0x12 应答](history/2026-09/2026-09-21-三项双端核对与一键清空不再干等应答.md)：小程序报的三条（连接并投屏后仍未连接 / 下载超时英文提示 / 一键清空设备已刷默认图还在转圈）逐条核对：**App 只有第 3 条**（前两条 App 本来就先连再进预览、下载失败走本地化文案）。`clearDeviceMemory` 改走 `ClearWatch.deleteAll`：等 0x12 应答期间 5 秒起、3 秒一拍回读 0x01，掩码清空即收尾并 `FrameBleClient.cancelPending` 收回应答；新增 4 条用例。⚠️ 未编译未真机。
- [2026-09-18 投屏旋转排查：不是圆形改动](history/2026-09/2026-09-18-投屏旋转排查-不是圆形改动.md)：反馈「iOS 端 370 的旋转没正确应用、投到设备上又反过来」。**先排除 `be1268f`（圆形）**：它只有 3 个控件树 hunk + 1 个字段解析，`roundScreen` 全仓 3 个读取点全在预览控件树里，出图链路（`_bake` / `coverCropToSize` / 抖动接口 / BLE）零引用，且 `shapeType != 1` 时与改动前逐字节一致；整条投屏链路自 2026-09-03 之后再无其它提交。落点在**竖向导出角**（唯一来源是后端 `verticalRotation`，缺省 0 = 不转，2026-08-04 口径），预览本就不显示导出角，所以只有设备上看得出来。本轮①加排查日志（进页面打设备字段与导出角、每张出图补 `导出角=`），②修掉一条确实丢角度的路径 ——`refreshDeviceDetail` 原来只把 5 个固件字段搬回列表设备，`verticalRotation`/`shapeType` 是 `final`、详情带回来也被丢掉；改成可变并**只在详情真的带了这个键时**才覆盖（0 是合法角度，不能与缺失混为一谈）。⚠️ 本机无 Flutter 工具链，未编译未真机。
- [2026-09-18 星币不足弹窗显示余额与「去充值」](history/2026-09/2026-09-18-星币不足弹窗显示余额与去充值.md)：产品要「先显示当前剩余星币，再在『我知道了』右边加一颗『去充值』」。弹窗正文下面补一条通栏余额带（走 `AppDialog` 的 `content` 位，左标签右数字），「知道了」右边补「去充值」→ `AppRoutes.starCoin`，回来重取余额。⚠️ **iOS 不画那颗按钮**——那一端整块星币模块藏着（`moduleHiddenOnThisApp`），路由只给兜底空页，点过去是一片空白；余额带照常显示（那是「AI 还能用多少」的信息）。另两条：① **弹窗弹出来之前先把余额等回来**（`showDialog` 的 builder 只构建一次，后到的余额不会重绘它；🔶 小程序靠 `setData` 自动更新，不必等——两端用户看到的都是最新余额，只是路径不同）；② `_loadTokenBalance` **读不到余额时不再抹成 `--`**（`fetchBalance` 失败返回 null，以前没人看见这个字段，现在它摆在弹窗上）。文案新增 `aiTokenBalanceLabel` / `aiTokenGoRecharge` 四语种。**同日追加**：产品定「**跳到套餐确认**」，于是 `pushNamed(starCoin, arguments: true)` → `StarCoinPage(autoOpenPurchase: true)` → 该页拉完套餐自己 push 确认页（⚠️ 不从聊天页直接 push 确认页：那页要一个具体 `StarPackage`，列表在星币管理页拉、确认页 08-28 起刻意不自己再拉一遍；两跳还让返回栈里留着管理页可换档。`_autoOpened` 只兑现一次，否则买完回来 `_load` 再跑会把用户又推进去）。⚠️ **顺带对齐默认档位 0 → 1**：小程序是 `packages[1] || packages[0]`（设计稿高亮的主推档），App 原来是第一档、注释却写着「与小程序一致」——直达确认页之后这个差异会变成「两端确认不同的套餐」。⚠️ 本机无 Flutter 工具链，analyze/test/编译/真机全未跑（新增的 l10n 用例也没跑）；借 flowerpot 的四个 Dart 静态自检跑了 167 个文件全绿。**小程序侧同口径用例真跑过**（60 个文件全过，含新增的 `token-auto-confirm` 4 条）。
- [2026-09-18 iOS 先屏蔽星币管理（模块 + 入口）](history/2026-09/2026-09-18-iOS先屏蔽星币管理.md)：用户口径「iOS 端先屏蔽星币管理这个模块和入口」。开关**一处定义**——`StarPayType.moduleHiddenOnThisApp`（用 `defaultTargetPlatform` 而非 `Platform.isIOS`，界面判定要能被 widget 测试覆盖），⚠️ 与既有的 `supportedOnThisApp` 是**两个**开关不可合并：那条管「能不能买」（2026-08-27：iOS 付不了但页面照开、至少看得到价目表），这条管「露不露面」，合成一个将来 IAP 接好一改会连屏蔽一起解除。三个生效点：①「我的」页星币管理行不画，**连同其后 13 的间距**（只藏行会让「操作指南」上方多出空当），并跳过只服务这一行的余额兜底请求；② AI 会话页余额胶囊**余额照常显示**（那是"AI 还能用多少"的信息）但不再可点、末尾 `>` 一并去掉，免得画出"点了没反应"的可点感；③ 路由 `AppRoutes.starCoin` 兜底成**能返回的空页**（照搬 `bleDebug` 的写法，本工程无 `onUnknownRoute`，`return null` 会抛异常）。`star_coin_page.dart` 头上那段「让 iOS 至少看得到价目表」就地标注为已被本轮口径覆盖，别据此把屏蔽去掉。用例：`test/star_purchase_test.dart` 增一组按 `debugDefaultTargetPlatformOverride` 切端断言。⚠️ **本机无 Flutter 工具链**，analyze/test 未跑，需在能跑的机器上过一遍并在 iOS 上真机确认。

- [2026-09-18 横向导出角改取后端 `rotationDegree`](history/2026-09/2026-09-18-横向导出角改取后端rotationDegree.md)：后台该产品配的是横向 90°/竖向 180°，而 App 把**横向角写死成 270°**（小程序一直按后端字段走）——**整整差 180°**，正是「投到设备上反过来了」；竖向两端一致本来就没问题。导出角是给设备屏做的补偿、预览会按同角反向转回来，所以预览永远是正的、只有设备上看得出来。新增 `device_rotation.dart`（1:1 对应小程序 `utils/device-rotation.js`：横向缺省 270、竖向缺省 0，0 是合法角度），`DeviceItem` 加 `rotationDegree`，预览页横向分支改走它；没配该字段的产品逐度不变。⚠️ 本机无 Flutter 工具链，用例未运行，待真机确认。
- [2026-09-18 圆形逻辑整体回滚](history/2026-09/2026-09-18-圆形逻辑整体回滚.md)：圆屏现在还用不上，把 09-17 那轮加的 `_ScreenClip`/`_kPickupShadow`、`DeviceItem.shapeType`/`roundScreen`、`_deviceFromJson` 的解析与那份用例全部撤掉（反向应用补丁，`git diff be1268f^` 核对只剩 09-18 的旋转排查改动）；⚠️ 那轮本来就碰不到出图（`roundScreen` 3 个读取点全在控件树里），撤它不改变投屏行为，撤的理由是用不上的代码不留在主路径上。保留 09-18 的排查日志与 `verticalRotation` 详情合并。⚠️ 本机无 Flutter 工具链，未编译未真机。
- [2026-09-17 圆屏产品预览裁圆、导出仍方（**已于 2026-09-18 回滚**）](history/2026-09/2026-09-17-圆屏产品预览裁圆导出仍方.md)：后台「形状类型 = 圆形」（`shapeType` 1）此前 App 侧完全没消费（只在一行注释里）。产品定稿「预览裁成圆、导出还是方的」——设备屏幕是圆的、方框四角本来就显示不出来，预览裁圆才是所见即所得；帧数据仍按方形矩阵传（四角留白），`_bake` 一个字未改。`DeviceItem` 加 `shapeType`（可选、默认 0=方形）与 `roundScreen`（**只认 1**，没见过的值退回方形）；预览两处 clip 走新的 `_ScreenClip`（圆→`ClipOval`，方→原样 `ClipRRect` + 同一半径）。⚠️ **没动 `_deviceSize`**（它同时是导出画布尺寸、且与 BLE `screenCode` 绑定）——代价是圆屏产品的屏型进 `FrameProtocol.screenTypes` 之前，框会是椭圆而非正圆，需设备侧给屏型码与真实分辨率。⚠️ 本机无 Flutter SDK，analyze/test 均未跑；小程序侧同口径用例已真跑通。**小程序同日同口径已改**。
- [2026-09-10 协议换法务 20260909 全文](history/2026-09/2026-09-10-协议换法务20260909全文.md)：**两端同时改**（App 英文、小程序简中）。用户协议由 2026-5-13 的三节旧短文换成法务 15 章全文并改为固定英文（不再走 `AppL10n.pick`），隐私政策换成同批新版，AI 服务协议违规阶梯按产品口径改为「1st-3rd 只提示违禁、第 4 次起封 24 小时」（中英日三份都改）。生效日期、版本号笔误、英文断词 16 处、第 13 节公司名四件事已按产品答复统一修正（逐条列在记录里），AI 服务协议的公司名方括号占位符一并去掉；⚠️ 用户协议第 9 章的阶梯已按产品裁定对齐到 AI 服务协议（用例把两页钉在一起）；本机无 Flutter SDK，analyze/test 未跑。
