...
class FrameImageCodec {
  ...
  /// 六色调色板。黑/白保留纯值作无彩色锚点；黄/红/蓝/绿为「实拍校准值」。
  /// 标定来源：6色.jpg 拍屏，取每条色带中位 RGB
  ///   黑 27,14,33 · 白 201,203,199 · 黄 207,196,54 · 红 115,30,24 · 蓝 59,71,131 · 绿 66,95,77
  /// 再按实测黑/白点线性归一化到 0~255 工作空间：归一 =(实测−黑点)/(白点−黑点)×255。
  /// 重新校准步骤：传彩条 → 中性光拍屏 → 取色带中位 RGB → 套上面公式 → 替换下面四个彩色项。
  static const List<PaletteEntry> palette = [
    PaletteEntry(0x0, 0, 0, 0, '黑'), // 黑、白保留纯值作无彩色锚点
    PaletteEntry(0x1, 255, 255, 255, '白'),
    PaletteEntry(0x2, 255, 246, 32, '黄'),
    PaletteEntry(0x3, 129, 22, 0, '红'),
    PaletteEntry(0x5, 47, 77, 151, '蓝'),
    PaletteEntry(0x6, 57, 109, 68, '绿'),
  ];
