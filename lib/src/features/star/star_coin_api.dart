...
class StarCoinApi {
  ...
  static Future<StarOrder> createOrder({
    ...
    // ⚠️ **端上不校验 goodsId 的取值**（含 0）：合法范围由后端定，端上加一道「必须大于 0」
    // 的闸只会把后端将来放开的取值挡在门外。真发出去被拒时，后端的 retMsg 会照原样透出。
    debugPrint(
      '[星币] 建单 goodsId=${package.goodsId} name=${package.name} payType=$payType',
    );
