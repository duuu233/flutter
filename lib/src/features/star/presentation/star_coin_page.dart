...
/// 单张套餐卡（小程序 `.package-card`，208×248rpx = 104×124）。
///
...
class _PackageCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      onTap: onTap,
      child: SizedBox(
        // 2026-09-08 当日三次收窄：150 → 138 → 124 → 104（对齐小程序 208rpx）。
        // ⚠️ 150 这个加宽本来就是多余的：当时把「价格支持六位数值」读成了六位
        // **整数**（¥999999.99），产品口径其实是**连两位小数一共六位**，即整数
        // 部分四位、最大 ¥9999.99。主价格与原价两行都在 FittedBox 里，放不下自动
        // 缩、不会截断，所以这边没有硬宽度下限；真正的下限是缩到看不清。
        width: 104,
        height: 124,
        child: Stack(
