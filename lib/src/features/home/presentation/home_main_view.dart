...
class _HomeMainView extends StatelessWidget {
  ...
  /// [_entrySubtitleAndroidFinalDelta]（第四轮 +1、第五轮 +2、2026-09-02 两次回调各 −1 = 1），
  /// 所以真机上的安卓最终字号是 **7 ~ 9**，不是这个差值单独划出的 8 ~ 10。
  static const double _entrySubtitleAndroidDelta = 1;

  /// 安卓端副标题**两步都走完之后统一再降的字号数**。四轮需求累加：
  /// 2026-09-01 第四轮「副标题安卓端还要小一个字号」记 +1，
  /// 同日第五轮「副标题安卓端需要再小二个字号」再记 +2（合计 3），
  /// 2026-09-02 第六、七轮「副标题安卓端再大一个字号，现在太小了」各回调 −1 —— 现为 **1**。
  ///
  /// ⚠️ 走了一圈**又回到 2026-09-01 第四轮那一档**（安卓 7 ~ 9）：第五轮那两号是一次性
  /// 调过头（360dp 上中文被压到 6，只有设计稿 `.entry-desc` 18rpx≈9 的三分之二），
  /// 09-02 分两次退回来。口径始终没变，仍是「在前两步的结果上统一退档」，只有档数在动。
  /// 再往上就是 0 —— 那等于**取消**这一步、退回 09-01 第三轮（安卓 8 ~ 10、只比 iOS 小一号），
  /// 若还嫌小，该动的是 [_entrySubtitleMaxFontSize] 那条上限或基准，而不是把这个差值变成负数
  /// （负数会顶过前两步量出来的「放得下的字号」，中文重新折行、英文重新出「...」）。
  ///
  ...
  /// ⚠️ 把它**调小**（2026-09-02 那两次回调）同样安全：最终值只是更靠近前两步量出来的
  /// 那个「放得下的字号」，永远不会越过它，所以中文照旧单行、英文照旧两行完整 ——
  /// 这条只在差值仍 ≥ 0 时成立，见上面那段「再往上就是 0」。
  /// 下限随之回到 `基准 9 − 1 − 1 = 7`（只有窄屏/放大系统字体时才会落到那里）。
  ...
  ///
  /// **第三步（仅安卓，2026-09-01 第四、五轮需求：「副标题安卓端还要小一个字号」
  /// →「再小二个字号」，2026-09-02 第六、七轮「再大一个字号」两次回调）**：
  /// 把前两步的结果**统一再减** [_entrySubtitleAndroidFinalDelta]（四轮累加，现为 1）。
  ...
  ///    所以一路都不可能新增「...」—— 调小第三步的差值（2026-09-02 两轮）也只是更靠近它、不会越过。
  ///
  ...
  double _entrySubtitleFontSize(BuildContext context, List<String> subtitles) {
    ...
    // 第三步那一号（第四轮 +1、第五轮 +2、2026-09-02 两次回调各 −1 = 1）。
    ...
    // 第三步（仅安卓）：两步都走完之后统一再降 1 号（第四轮 +1、第五轮 +2、09-02 两次各 −1）——
    // 见 [_entrySubtitleAndroidFinalDelta]。
