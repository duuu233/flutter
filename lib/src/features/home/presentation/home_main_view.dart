...
class _HomeMainView extends StatelessWidget {
  ...
  /// [_entrySubtitleAndroidFinalDelta]（第四轮 1 + 第五轮 2 = 3），所以真机上的
  /// 安卓最终字号是 **5 ~ 7**，不是这个差值单独划出的 8 ~ 10。
  static const double _entrySubtitleAndroidDelta = 1;

  /// 安卓端副标题**两步都走完之后统一再降的字号数**。两轮需求累加：
  /// 2026-09-01 第四轮「副标题安卓端还要小一个字号」记 1，
  /// 同日第五轮「副标题安卓端需要再小二个字号」再记 2 —— 合计 **3**。
  /// 两轮的诉求一样：中文已经治成单行、英文已经是两行完整，产品复看真机后
  /// 仍嫌整体偏大，要的是**在当前结果上继续退档**，而不是改版式。
  ///
  ...
  /// 还是第二步（中文，单行），都稳定比上一轮小同样的档数，六张卡仍共用同一个值。
  ///
  /// ⚠️ 仍是**只减不加**，不会新增截断、也不会把中文重新顶成两行：
  /// 比一个「两行放得下」的字号更小必然还是两行放得下，
  /// 比一个「一行放得下」的字号更小必然还是一行放得下。
  /// 代价是下限跟着降到 `基准 9 − 1 − 3 = 5`（只有窄屏/放大系统字体时才会落到那里）。
  static const double _entrySubtitleAndroidFinalDelta = 3;
  ...
  /// →「再小二个字号」）**：把前两步的结果**统一再减**
  /// [_entrySubtitleAndroidFinalDelta]（两轮累加，现为 3）。
  /// 放在最后而不是并进前两步的差值里，理由写在那个常量上。
  ///
  /// 三条不变量，都是这个写法直接给出的：
  ///
  /// 1. **中文一定不换行**（只要 5 这一档放得下）—— 中文六条里最长的是
  ///    「拍摄照片并投屏」约 7 个字身宽，360dp 屏上副标题可用宽 ≈65，
  ///    第二步挑到 9（≈63 宽）、第三步再落到 6，仍是单行；
  ...
  double _entrySubtitleFontSize(BuildContext context, List<String> subtitles) {
    ...
    // 第三步那几号（2026-09-01 第四轮 1 + 第五轮 2）。iOS 恒为 0，所以下面所有出口都能无脑减。
    ...
    // 第三步（仅安卓）：两步都走完之后统一再降 3 号（第四轮 1 + 第五轮 2）——
