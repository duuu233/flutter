...
class _HomeMainView extends StatelessWidget {
  ...
  /// [_entrySubtitleAndroidFinalDelta]（第四轮 +1、第五轮 +2、2026-09-02 回调 −1 = 2），
  /// 所以真机上的安卓最终字号是 **6 ~ 8**，不是这个差值单独划出的 8 ~ 10。
  static const double _entrySubtitleAndroidDelta = 1;

  /// 安卓端副标题**两步都走完之后统一再降的字号数**。三轮需求累加：
  /// 2026-09-01 第四轮「副标题安卓端还要小一个字号」记 +1，
  /// 同日第五轮「副标题安卓端需要再小二个字号」再记 +2（合计 3），
  /// 2026-09-02 第六轮「副标题再大一个字号，现在太小了」回调 −1 —— 现为 **2**。
  /// 前三轮一路往下调，第五轮那两号把 360dp 上的中文压到 6、贴着可读性下沿，
  /// 产品真机复看后要回来一号；口径没变，仍是「在前两步的结果上统一退档」，
  /// 只是退的档数从 3 收到 2。
  ///
  ...
  /// ⚠️ 把它**调小**（2026-09-02 那次回调）同样安全：最终值只是更靠近前两步量出来的
  /// 那个「放得下的字号」，永远不会越过它，所以中文照旧单行、英文照旧两行完整。
  /// 下限随之从 5 回到 `基准 9 − 1 − 2 = 6`（只有窄屏/放大系统字体时才会落到那里）。
  ...
  /// →「再小二个字号」，2026-09-02 第六轮「再大一个字号」回调）**：
  /// 把前两步的结果**统一再减** [_entrySubtitleAndroidFinalDelta]（三轮累加，现为 2）。
  ...
  ///    所以一路都不可能新增「...」—— 调小第三步的差值（2026-09-02）也只是更靠近它、不会越过。
  ///
  ...
  double _entrySubtitleFontSize(BuildContext context, List<String> subtitles) {
    ...
    // 第三步那几号（第四轮 +1、第五轮 +2、2026-09-02 回调 −1 = 2）。
    ...
    // 第三步（仅安卓）：两步都走完之后统一再降 2 号（第四轮 +1、第五轮 +2、09-02 回调 −1）——
