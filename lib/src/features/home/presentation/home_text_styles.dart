...
class _HomeTextStyles {
  ...
  //      英文照旧两行完整；
  //      ⚠️ 2026-09-02 第六、七轮需求「副标题安卓端再大一个字号，现在太小了」，
  //      同一个常量 3 → 2 → **1**（第五轮那两号一次性调过头，把 360dp 上的中文压到 6，
  //      分两次退回来 —— 等于又回到 09-01 第四轮那一档）。回调同样安全：
  //      最终值只是更靠近前两步量出来的「放得下的字号」、不会越过它。
  //      再往上就是 0（取消这一步、退回 09-01 第三轮），要更大就得动上限/基准，
  //      而不是把这个差值变成负数。
